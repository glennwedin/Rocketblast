...
export default class Level {
    constructor(levelsettings) {
        var currentSettings =
            levelsettings.levels['level_' + BOOM.player.getCurrentLevel()];
        this.starttime = 0;
        this.time = currentSettings.time * 1000;
        this.lifemax = 100;
        this.life = 100;
        this.score = 0;
        ...
        document.getElementById('bg').style.backgroundImage =
            'url(/gfx/bg/' + currentSettings.bg + ')';
    }

    showHud() {
        this.hud.score.style.display = '';
        this.hud.time.style.display = '';
        this.hud.lifebar.style.display = '';
    }

    hideHud() {
        this.hud.score.style.display = 'none';
        this.hud.time.style.display = 'none';
        this.hud.lifebar.style.display = 'none';
    }

    start() {
        BOOM.audio.effects.hit.play();
        BOOM.audio.effects.hit.pause();
        console.log('kjører ikke denne?');
            ...
            this.dialog(
                {
                    ...
                    text: 'You saved the party. Now move along to the next one',
                    action_yes: 'GO!',
                    action_next: null,
                    action_previous: null
                },
                function() {
                    // Setter player-level 1up
                    BOOM.player.increaseLevel();
                    BOOM.init(false);
        ...
        if (th.touching) {
            ...
            th.bombgroups.forEach(function(bombgroup, index) {
                ...
                bombgroup.bombs.forEach(function(bomb, index) {
                    ...
                    if (
                        bomb.pos.x <
                            th.touches.x[th.touches.x.length - 1] +
                                bomb.settings.width &&
                        bomb.pos.x >
                            th.touches.x[th.touches.x.length - 1] -
                                bomb.settings.width &&
                        bomb.pos.y <
                            th.touches.y[th.touches.y.length - 1] +
                                bomb.settings.height &&
                        bomb.pos.y >
                            th.touches.y[th.touches.y.length - 1] -
                                bomb.settings.height
                    ) {
